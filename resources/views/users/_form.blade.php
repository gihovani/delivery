...
<div class="form-group row">
    ...
    <div class="col-md-7">
        {!! Form::text('telephone',$user->telephone ?? '(48) ',['required' => true, 'disabled' => (isset($disabled)), 'autocomplete' => 'phone', 'class'=>'form-control input-telephone'.($errors->has('telephone') ? ' is-invalid' : '')]) !!}
...
<div class="form-group row">
    ...
    <div class="col-md-7">
        ...
        <div class="form-check">
            {!! Form::checkbox('no-email', 1, '', ['class'=>'form-check-input no-email', 'id' => 'no-email']) !!}
            {!! Form::label('no-email', __('No Email'),['class'=>'form-check-label']) !!}
        </div>
    </div>
</div>
<div class="form-group row{{isset($hide_profile) ? ' d-none' : ''}}">
    ...
    <div class="col-md-7">
        {!! Form::select('roles[]',\App\User::rolesToOptionList(), isset($user->roles) ? explode(',', $user->roles) : [\App\User::ROLE_CUSTOMER],['required' => !isset($hide_profile), 'multiple' => true, 'disabled' => (isset($disabled)), 'class'=>'form-control input-roles'.($errors->has('roles') ? ' is-invalid' : '')]) !!}
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        $('.no-email').on('change', function () {
            ...
            var telephone = $('.input-telephone').val().replace(/\D/g, '');
            var email = '';
            if ($(this).prop('checked')) {
                email = telephone+'{{App\User::DEFAULT_EMAIL}}';
            }
            $email.val(email);
        });
